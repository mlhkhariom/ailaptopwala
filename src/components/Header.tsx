    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-[70] transition-all duration-500 ${
          scrolled ? "bg-card/95 backdrop-blur-xl shadow-lg border-b border-border/50" : "bg-transparent backdrop-blur-md"
        }`}
      >
        <nav className="container mx-auto flex items-center justify-between px-4 py-2.5 md:py-3">
          ...
          {/* Mobile toggle */}
          <button
            className="lg:hidden relative z-[80] flex items-center justify-center w-10 h-10 rounded-xl bg-muted text-foreground active:scale-95 transition-transform"
      ...
      {/* Mobile fullscreen overlay menu — OUTSIDE header for proper stacking */}
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className="lg:hidden fixed inset-0 z-[75] flex flex-col items-center justify-center gap-2 px-8"
