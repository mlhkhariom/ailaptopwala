  ...
  return (
    <div className="p-4 md:p-8 h-full flex flex-col animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl sm:text-4xl font-bold tracking-tight flex items-center gap-4">
            Live Chat<span className="text-gradient">.</span>
            <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-success/10 border border-success/20 text-success text-xs sm:text-sm font-semibold">
              <Radio className="w-4 h-4 animate-pulse" />
              Connected
            </span>
          </h1>
          <p className="text-muted-foreground mt-2 text-sm sm:text-base">Test your AI sales agent in real-time</p>
        </div>
      </div>

      {/* Chat Container */}
      <div className="flex-1 glass-card flex flex-col overflow-hidden glow-accent min-h-0">
        {/* Chat Header */}
        <div className="p-4 sm:p-5 border-b border-border/50 bg-gradient-to-r from-accent/5 to-transparent flex items-center gap-4">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br from-accent/20 to-accent/10 flex items-center justify-center">
            <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 text-accent" />
          </div>
          <div>
            <h3 className="font-bold text-base sm:text-lg">AI Sales Agent</h3>
          ...
          <div className="space-y-6">
            {messages.map((message, i) => (
              ...
              >
                ...
                <div
                  className={cn(
                    "max-w-[80%] rounded-2xl px-4 py-3 sm:px-5 sm:py-4",
        ...
        {/* Input */}
        <div className="p-4 sm:p-5 border-t border-border/50 bg-card/30">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleSend();
