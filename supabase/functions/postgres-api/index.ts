...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'get-chats':
        result = await client.queryObject(`
          SELECT id, contact_uid, role, content, image_url, created_at, mobile_number, name
          FROM chat_messages 
      ...
      case 'get-contacts':
        ...
        try {
          result = await client.queryObject(`
            SELECT uid as contact_uid, name, phone_number, email, created_at, updated_at
            FROM contacts 
      ...
      case 'send-message':
        ...
        const imageUrl = messageData.image_url || messageData.imageUrl || null;
        const mobileNumber = messageData.mobile_number || messageData.mobileNumber || null;
        
        console.log('Extracted values:', { imageUrl, mobileNumber });
        
        result = await client.queryObject(
          `INSERT INTO chat_messages (contact_uid, content, role, image_url, created_at, mobile_number) 
           VALUES ($1, $2, 'assistant', $3, NOW(), $4) RETURNING *`,
          [messageData.contactId, messageData.message, imageUrl, mobileNumber]
        );
      ...
      case 'delete-message':
        ...
        return jsonResponse({ success: true, deleted: result.rows[0] });

      case 'update-message':
        const updateData = data || {};
        result = await client.queryObject(
          `UPDATE chat_messages SET content = $1, image_url = $2 WHERE id = $3 RETURNING *`,
          [updateData.content, updateData.image_url, updateData.messageId]
        );
        return jsonResponse({ success: true, updated: result.rows[0] });

      case 'update-contact-name':
        // Update name for all messages with specific mobile number
        const nameData = data || {};
        result = await client.queryObject(
          `UPDATE chat_messages SET name = $1 WHERE mobile_number = $2 RETURNING *`,
          [nameData.name, nameData.mobile_number]
        );
        console.log(`Updated ${result.rowCount} rows with name: ${nameData.name}`);
        return jsonResponse({ success: true, updated: result.rowCount, rows: result.rows });
