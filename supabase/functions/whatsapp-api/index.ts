...
interface SendMessageRequest {
  ...
  header_image?: string; // Added for image support
  field_1?: string;
  field_2?: string;
  field_3?: string;
  field_4?: string;
...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case 'send-message': {
        const { phone_number, message_body, template_name, template_language, header_image } = data as SendMessageRequest;
        ...
        if (header_image) {
          requestBody.header_image = header_image;
          // If there's no message body, WhatsApp requires one, even for images.
          // An empty string might not work, so a zero-width space is safer.
          if (!requestBody.message_body) {
            requestBody.message_body = ' ';
          }
        }
        
        // Ensure there is something to send
        if (!requestBody.message_body && !requestBody.template_name) {
           return new Response(
            JSON.stringify({ error: 'Cannot send a message with no content (text or template).' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
      ...
      case 'send-media-message': {
        const { phone_number, media_type, media_url, caption } = data;

        if (!phone_number || !media_url) {
          return new Response(
            JSON.stringify({ error: 'Phone number and media URL are required' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
        ...
        const apiUrl = `${apiBaseUrl}/${vendorUid}/contact/send-media-message?token=${apiToken}`;
        
        console.log('Sending WhatsApp media message to:', cleanPhone);
        ...
        const responseText = await response.text();
        console.log('WhatsApp API media response:', responseText);
